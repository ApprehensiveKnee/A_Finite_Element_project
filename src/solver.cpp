...
void serialSolver::setup(const std::string &file_name, const bool &option)
{
    ...
    // PLEASE NOTE:
    // In the serial setting it isn't compulsory to initially insert all of the non zero elements
    // in the gloabal matrix. This is done in 2 parallel versions of the code, since, in those, the threads
    // insert new non zero elements in the SparseMatrix, which is stored in a compressed format and thus
    // a dinamic structure: this creates some problems when the threads try to insert new elements in the
    // same position of the matrix. In the serial setting this is not a problem, since the contributions of
    // the local matrixes are progressively computed and then stored in the global matrix serially.
    // Still, to better ascertain the performance of the parallel versions, we decided to use the same
    // approach in the serial setting as well, only when the performance_comparison option is selected.
    // This allows to have a more fair comparison between the serial and the parallel versions, especially
    // regarding the assemble method.
    // Non the less, it is important to notice that this approach is not the most efficient one in the serial,
    // while this overhead should be taked into consideration when comparing the performance of the parallel.
...
void serialSolver::_apply_boundary()
{
    ...
    for (const Element<DIM> &rect : _mesh.getElems())
    {
        ...
        for (unsigned int p = 0; p < _dof.dof_per_cell() && rect.getBound(); p++)
        {
            ...
            if (bound)
            {
                _system_mat.prune([&gindex](int i, int j, float)
                                  { return i != gindex; });
