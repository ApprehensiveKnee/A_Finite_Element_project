...
3. Finally, the <span style="color:darkorange">**third approac**h</span> revolves around a <span style="color:darkorange">_**coloring algorithm**_</span> based on graph theory: the elements are assigned to some color, in such a way that neighboring elements do not share the same color. Having divided the elements in such a way, the ones belonging to the same group can be processed in parallel, as their contribution to the global matrix do not map on the same coefficients, which means that **there can be no possibility of race conditions occurring**. \
